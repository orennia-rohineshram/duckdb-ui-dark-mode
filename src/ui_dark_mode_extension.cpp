...
struct DarkModeProxy {
	...
	static std::string Start(uint16_t dark_port, uint16_t ui_port,
		...
		proxy->local_url =
		    StringUtil::Format("http://localhost:%d/", dark_port);

		auto upstream =
		    StringUtil::Format("http://localhost:%d", ui_port);

		// Catch-all: proxy every request to the upstream duckdb-ui server.
		auto *p = proxy.get();
		proxy->server.Get(
		    "/.*",
		    [p, upstream](const httplib::Request &req, httplib::Response &res) {
		...
		proxy->server.Post(
		    "/.*",
		    [upstream](const httplib::Request &req, httplib::Response &res,
			    ...
			    headers.emplace(
			        "Origin",
			        StringUtil::Format("http://localhost:%d",
			                           DEFAULT_UI_PORT));

			    auto ct = req.get_header_value("Content-Type");
			    auto result =
			        client.Post(req.path, headers, body, ct);
			    if (!result) {
				    res.status = 502;
				    res.set_content(
				        "Dark mode proxy: upstream not reachable",
				        "text/plain");
				    return;
			    }
			    res = result.value();
		...
		proxy->thread = duckdb::make_uniq<std::thread>(
		    [p]() { p->server.listen("localhost", p->dark_port); });
...
static unique_ptr<GlobalTableFunctionState> RunOnceInit(ClientContext &,
                                                        TableFunctionInitInput &) {
	return make_uniq<RunOnceState>();
}

static unique_ptr<FunctionData>
SingleStringBind(ClientContext &, TableFunctionBindInput &input,
...
// ---------------------------------------------------------------------------
static void StartDarkUIFunc(ClientContext &context, TableFunctionInput &input,
                            DataChunk &output) {
	...
	auto msg = system(cmd.c_str())
	               ? StringUtil::Format("Dark UI started at %s", url)
...
// stop_dark_ui()
// ---------------------------------------------------------------------------
static void StopDarkUIFunc(ClientContext &, TableFunctionInput &input,
                           DataChunk &output) {
	auto &state = input.global_state->Cast<RunOnceState>();
	if (state.done) {
		output.SetCardinality(0);
	...
	output.SetCardinality(1);
	output.SetValue(0, 0,
	                Value(was_running ? "Dark UI proxy stopped"
	                                 : "Dark UI proxy was not running"));
}
...
static void LoadInternal(ExtensionLoader &loader) {
	...
	// Settings
	config.AddExtensionOption(SETTING_DARK_PORT,
	                          "Port for the dark mode proxy server",
	                          LogicalType::USMALLINT,
	                          Value::USMALLINT(DEFAULT_DARK_PORT));

	config.AddExtensionOption(
	    SETTING_DARK_MODE,
	    "Dark mode: 'auto' (follow OS), 'on' (always dark), 'off' (disabled)",
	    LogicalType::VARCHAR, Value("auto"));

	// Table functions
	{
		TableFunction tf("start_dark_ui", {}, StartDarkUIFunc,
		                 SingleStringBind, RunOnceInit);
		loader.RegisterFunction(tf);
	}
	{
		TableFunction tf("stop_dark_ui", {}, StopDarkUIFunc,
		                 SingleStringBind, RunOnceInit);
		loader.RegisterFunction(tf);
	}
}
